...
const problems = [
  ...
  { id: 'reverse-string', title: 'Reverse String', difficulty: 'Easy', category: 'Strings', tags: ['Two Pointers', 'String'], companies: ['Apple', 'Microsoft'], acceptanceRate: 76, statement: 'Write a function that reverses a string. The input string is given as an array of characters s. You must do this by modifying the input array in-place with O(1) extra memory.', constraints: ['1 <= s.length <= 10^5', 's[i] is a printable ascii character'], examples: [{ input: 's = ["h","e","l","l","o"]', output: '["o","l","l","e","h"]' }], hints: ['Try using the two pointers approach.', 'Swap elements from start and end, moving towards the center.'] },
  ...
  { id: 'lru-cache', title: 'LRU Cache', difficulty: 'Hard', category: 'Design', tags: ['Hash Map', 'Linked List', 'Design'], companies: ['Google', 'Meta', 'Amazon'], acceptanceRate: 40, statement: 'Design a data structure that follows the constraints of a Least Recently Used (LRU) cache. Implement the LRUCache class with get(key) and put(key, value) operations in O(1) average time complexity.', constraints: ['1 <= capacity <= 3000', '0 <= key, value <= 10^4', 'At most 2 * 10^5 calls will be made to get and put'], examples: [{ input: 'capacity = 2, put(1,1), put(2,2), get(1), put(3,3), get(2)', output: 'returns 1, evicts key 2, returns -1' }], hints: ['Use a doubly linked list combined with a hash map.', 'The doubly linked list maintains the usage order, while the hash map provides O(1) access to nodes.'] }
];
